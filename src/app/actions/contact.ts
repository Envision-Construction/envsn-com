 ...
 *   2. Reject if honeypot field is populated (bot signal)
 *   3. Verify Cloudflare Turnstile token against their siteverify endpoint
 *   4. Send email via Resend to LEADS_EMAIL_TO
 *
 * Environment variables required for the action to actually send:
 *   - RESEND_API_KEY
 *   - LEADS_EMAIL_TO
 *   - LEADS_EMAIL_FROM (must be on a verified Resend domain)
 *   - TURNSTILE_SECRET_KEY
 *
 * If any are missing, the action returns a clear error so the UI can show
 * "Form not yet configured" instead of failing silently.
 */
...
async function verifyTurnstile(token: string, remoteIp?: string) {
  ...
  if (!secret) {
    return { success: false, reason: 'TURNSTILE_SECRET_KEY not set' }
  }
